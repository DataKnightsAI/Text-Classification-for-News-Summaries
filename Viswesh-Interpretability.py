...
cv = CountVectorizer(min_df = 2, lowercase = True, token_pattern=r'(?u)\b[A-Za-z]{2,}\b', 
                        strip_accents = 'ascii', ngram_range = (4, 7), 
                        stop_words = 'english')
...
eli5.show_weights(estimator = logreg_model, top = 10, target_names = ['W','S','B','T'], feature_names = x_train_bagofwords.columns.tolist())
#eli5.show_prediction(estimator = logreg_model, doc = x_test_bagofwords.values[7599], 
